...
input_LHS_t = pd.read_csv('inputs_LHS_8.data')
...
for sce_i in range (sce_number):
	...
	for rep_runs in range(len(input_LHS_t)):
		...
		if sce_i == 8:
			...
			# input LHS
			shared = input_LHS_t.iloc[rep_runs]["shared"]
			creation = input_LHS_t.iloc[rep_runs][" creation"]
			input_LHS = [shared, creation]
