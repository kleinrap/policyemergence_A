...
\item A \emph{type} corresponds the agent type: policy maker or policy entrepreneur. This will go on to define the actions possible for the agent.
...
For every step, the agents can interact with one another. These interactions are focused on the causal beliefs and the preferred states. Each agents considers all of the actions that it can perform, grading them, and then deciding which actions will be performed depending on the expected impact of that action.

The assessment of the interaction is based on the conflict level between agents. Following the literature, it is assumed that agents with an average conflict level are most likely to interact, followed by a low and then high conflict level. A conflict level can be calculated for every belief in the belief system of the agents.

The causal belief interaction performed is then provided by the following equation:
...
And similarly for the preferred state interactions:
...
For the interactions, the likelihood of performing an action ...
...
\item Agenda setting step:
	\begin{enumerate}
	\item {\bfseries (+PL/Co/PI/PK)}
	
	\emph{Resources distribution:} The agents see their resources replenished.
	
	\item \emph{Preferences calculation (issues):} Each agent calculates the preference for their deep core and policy core issues. Additionally, each agents selects an issue that (s)he will advocate for in his/her policy core issues based on the preferences.
	
	\item {\bfseries (+PL/Co)}
	
	\emph{Agent interactions:} The agents influence one another to advance their interest relating to the policy core issues, spending all of their resources.
	
	\item {\bfseries (+PL/Co/PI/PK)}
	
	\emph{Preferences calculation - policy makers (issues):} Policy makers update their preferences before the selection of the agenda to take into account the agent interactions.
...
\item Policy formulation step:
	...
	\emph{Agent interactions:} The agents influence one another to advance their interest relating to the policy instruments, spending all of their resources.

	\item {\bfseries (+PL/Co/PI/PK)}
	
	\emph{Preferences calculation - policy makers (policy instruments):} Policy makers update their preferences before the selection of the policy instrument to take into account the agent interactions.
...
\texttt{affiliation} is the political entity the agent identifies with,  
\texttt{advocacy} is the list of the issues the agent is supporting,
\texttt{resources} corresponds to the amount of resources the agents have {\bfseries (+PL/Co)}.

\item A \emph{type} corresponds the agent type: policy maker, policy entrepreneur {\bfseries (+PL)} or external parties {\bfseries (+PI)}. This will go on to define the actions possible for the agent.
...
The coalitions have the following attributes attributes {\bfseries (+Co)}:
